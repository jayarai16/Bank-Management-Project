...
class Bank:
    ...
    def balanceequiry(self):
        ...
        print(f"{self.__username} Blance is {temp[0][0]}")
    ...
    def fundtransfer(self, recive, amount):
        temp = db_query(f"SELECT balance FROM customer WHERE username= '{self.__username}'")
        if amount > temp[0][0]:
            print("Insufficient balance please deposite money")
        else:
            temp2 = db_query(f"SELECT balance FROM customer WHERE account_number= '{recive}")
            test1 = temp[0][0] - amount
            test2 = amount + temp2[0][0]

            db_query(f"UPDATE customers SET balance = '{test1}' WHERE username = '{self.__username}' ")
            db_query(f"UPDATE customers SET balance = '{test2}' WHERE account_number = '{recive}' ")
            receiver_username = db_query(f"SELECT username FROM customers where account_number ='{recive}' ")
            self.balanceequiry()
